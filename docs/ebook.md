...
The Critic is the pipeline's independent review agent. Its purpose is to improve the quality of the Actor's output through a second perspective that the Actor cannot provide for itself. It receives the `EPACImplementation` and evaluates it against the original `EPACSpec`, the approved `EPACPlan`, security standards, and organizational policy, then produces the `EPACReview`: a formal findings report in SARIF 2.1.0 format.

When the Critic returns the Actor for revision, it is not a failure of the pipeline. It is the pipeline working correctly. A finding caught by the Critic before Gate 2 is a finding that did not reach production. The iteration is the quality mechanism.
...
REJECT means the Critic identified one or more blocking findings. The `EPACReview` returns to the Actor for remediation. The Actor must address the specific findings and produce a new `EPACImplementation`. The Critic then reviews again. This loop continues until the Critic passes the implementation or the maximum iteration count is reached and the run is escalated to the Expert.

The Critic's recommendation is advisory. Only the Expert can make the final approval decision at Gate 2. A thorough Critic that writes specific, actionable feedback produces better final outputs than one that simply blocks with vague findings. The goal of the REJECT path is a better implementation, not a blocked one.
...
Install the base package with the recommended extras:

```bash
pip install "epac[langgraph,litellm]"
```

To install all available extras (including additional static analysis tools and storage backends):
...
The `EPACConfig` object controls the pipeline's behavior. Key parameters:
